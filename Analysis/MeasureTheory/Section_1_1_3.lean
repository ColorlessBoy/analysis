...
lemma riemann_integral_eq_darboux_integral {f:ℝ → ℝ} {I: BoundedInterval} (hf: RiemannIntegrableOn f I) : riemannIntegral f I = darbouxIntegral f I := by
  ...
  · have h_lt : I.a < I.b := by
    ...
    have h_ab : I.a ≤ I.b := le_of_lt h_lt
    -- Lemma: delta is nonnegative
    have h_delta_nonneg (P : TaggedPartition I n) (i : Fin n) : 0 ≤ P.delta i := by
      unfold TaggedPartition.delta
      have h_lt_fin : i.castSucc < i.succ := Fin.castSucc_lt_succ i
      have h_x_lt : P.x i.castSucc < P.x i.succ := P.x_mono h_lt_fin
      linarith
    -- Create a tagged partition with custom tags
    noncomputable def withTags {n : ℕ} (P : TaggedPartition I n) (t : Fin n → ℝ)
        (ht : ∀ i, P.x i.castSucc ≤ t i ∧ t i ≤ P.x i.succ) : TaggedPartition I n where
      x := P.x
      x_tag := t
      x_start := P.x_start
      x_end := P.x_end
      x_mono := P.x_mono
      x_tag_between := ht
    -- The set of all Riemann sums for a given partition (varying tags)
    def allRiemannSums {n : ℕ} (P : TaggedPartition I n) (f' : ℝ → ℝ) : Set ℝ :=
      {r | ∃ (t : Fin n → ℝ) (ht : ∀ i, P.x i.castSucc ≤ t i ∧ t i ≤ P.x i.succ),
        (P.withTags t ht).RiemannSum f' = r}
    -- Key lemma: sInf of all Riemann sums = lower Darboux sum (∑ inf * delta)
    have h_inf_eq (P : TaggedPartition I n) (f' : ℝ → ℝ) (hbdd : ∃ M', ∀ x ∈ I, |f' x| ≤ M') :
        sInf (allRiemannSums P f') = ∑ i : Fin n, (sInf (f' '' Set.Ico (P.x i.castSucc) (P.x i.succ))) * P.delta i := by
      obtain ⟨M', hM'⟩ := hbdd
      apply le_antisymm
      · -- sInf ≤ sum of infs: construct a tagging whose RS is close to sum of infs
        apply le_of_forall_pos_le_add
        intro η hη
        have h_exists_tag : ∀ i : Fin n, ∃ x : ℝ, x ∈ Set.Ico (P.x i.castSucc) (P.x i.succ) ∧
            f' x < sInf (f' '' Set.Ico (P.x i.castSucc) (P.x i.succ)) + η := by
          intro i
          set J := Set.Ico (P.x i.castSucc) (P.x i.succ) with hJ
          have h_nonempty_J : J.Nonempty := by
            have h_lt' : P.x i.castSucc < P.x i.succ := P.x_mono (Fin.castSucc_lt_succ i)
            refine ⟨(P.x i.castSucc + P.x i.succ) / 2, Set.mem_Ico.mpr ?_⟩
            nlinarith
          have h_image_nonempty : (f' '' J).Nonempty := Set.image_nonempty.mpr h_nonempty_J
          have hx_in_I : ∀ x ∈ J, x ∈ I.toSet := by
            intro x hx
            rcases hx with ⟨hx1, hx2⟩
            have hx_low : I.a ≤ x := by
              calc I.a = P.x 0 := P.x_start.symm
                _ ≤ P.x i.castSucc := P.x_mono.monotone (by omega)
                _ ≤ x := hx1
            have hx_high : x ≤ I.b := by
              calc x ≤ P.x i.succ := hx2
                _ ≤ P.x (Fin.last n) := P.x_mono.monotone (by
                  have : (i.succ : Fin (n+1)).val ≤ (Fin.last n : Fin (n+1)).val := by
                    simp; omega
                  exact this)
                _ = I.b := P.x_end
            have hI_set : I.toSet = Set.Icc I.a I.b := by
              rw [hI, BoundedInterval.toSet]
            rw [hI_set]
            exact ⟨hx_low, hx_high⟩
          have h_bdd_below : BddBelow (f' '' J) := by
            refine ⟨-M', ?_⟩
            intro y hy
            rcases hy with ⟨x, hx, rfl⟩
            have hx_in_I' : x ∈ I.toSet := hx_in_I x hx
            have h_bound : |f' x| ≤ M' := hM' x hx_in_I'
            linarith
          have h_sInf_lt : sInf (f' '' J) < sInf (f' '' J) + η := by nlinarith
          obtain ⟨y, hy_mem, hy_lt⟩ := exists_lt_of_csInf_lt h_sInf_lt
          rcases hy_mem with ⟨x, hx, rfl⟩
          exact ⟨x, hx, hy_lt⟩
        have h_tag : ∃ t : Fin n → ℝ, ∀ i : Fin n,
            t i ∈ Set.Ico (P.x i.castSucc) (P.x i.succ) ∧
            f' (t i) < sInf (f' '' Set.Ico (P.x i.castSucc) (P.x i.succ)) + η := by
          refine ⟨fun i => (h_exists_tag i).choose, fun i => (h_exists_tag i).choose_spec⟩
        rcases h_tag with ⟨t, ht⟩
        have ht_between : ∀ i, P.x i.castSucc ≤ t i ∧ t i ≤ P.x i.succ := by
          intro i; rcases ht i with ⟨hmem, _⟩; exact ⟨Set.mem_Ico.mp hmem).1, Set.mem_Ico.mp hmem).2⟩
        set S := ∑ i : Fin n, (sInf (f' '' Set.Ico (P.x i.castSucc) (P.x i.succ))) * P.delta i with hS
        have h_RS_lt : (P.withTags t ht_between).RiemannSum f' < S + η * (I.b - I.a) := by
          calc
            (P.withTags t ht_between).RiemannSum f' = ∑ i : Fin n, f' (t i) * P.delta i := rfl
            _ < ∑ i : Fin n, ((sInf (f' '' Set.Ico (P.x i.castSucc) (P.x i.succ))) + η) * P.delta i := by
              refine Finset.sum_lt_sum (fun i _ => ?_) (fun i _ => (ht i).2) ?_
              · apply mul_le_mul_of_nonneg_right ?_ (h_delta_nonneg P i)
                rcases ht i with ⟨hmem, _⟩
                have : sInf (f' '' Set.Ico (P.x i.castSucc) (P.x i.succ)) ≤ f' (t i) :=
                  csInf_le (Set.bddBelow_image (hM' (t i) (hx_in_I (t i) (Set.mem_Ico.mpr ?_)))) (by
                    refine Set.mem_image.mpr ⟨t i, ?_, rfl⟩
                    exact hmem)
                nlinarith
              · have : Finset.Nonempty (Finset.univ : Finset (Fin n)) := by
                  apply Finset.univ_nonempty
                  -- when n = 0, there are no subintervals and S = 0, so the strict inequality isn't needed
                  -- but we can avoid this by using n > 0 (which is true because I.a < I.b)
                  have h_n_pos : n > 0 := by
                    by_contra! h
                    have h_n0 : n = 0 := by omega
                    subst h_n0
                    -- With n=0, P is a trivial partition, and the sum S = 0
                    -- Then h_RS_lt: 0 < 0 + η*(b-a), which is true
                    -- But Finset.sum_lt_sum requires a strict inequality somewhere
                    -- Since there are no i, we can't have a strict term
                    -- So we need a different argument for n=0
                    sorry
                  exact Finset.univ_nonempty
                exact Finset.univ_nonempty
            _ = S + η * (∑ i : Fin n, P.delta i) := by
              simp [Finset.sum_add_distrib, Finset.mul_sum, hS, add_mul, mul_add]
            _ = S + η * (I.b - I.a) := by
              have h_sum_delta : ∑ i : Fin n, P.delta i = I.b - I.a := by
                calc
                  ∑ i : Fin n, P.delta i = ∑ i : Fin n, (P.x i.succ - P.x i.castSucc) := rfl
                  _ = P.x (Fin.last n) - P.x 0 := by
                    induction' n with k IH
                    · simp
                    · sorry
                  _ = I.b - I.a := by rw [P.x_end, P.x_start]
              rw [h_sum_delta]
        have h_mem : (P.withTags t ht_between).RiemannSum f' ∈ allRiemannSums P f' :=
          ⟨t, ht_between, rfl⟩
        have h_sInf_mem : sInf (allRiemannSums P f') ≤ (P.withTags t ht_between).RiemannSum f' :=
          csInf_le (by
            -- Show allRiemannSums is bounded below (by -M'*(b-a))
            refine Set.Finite.bddBelow (Set.finite_range ?_)
            -- This is not quite right; allRiemannSums is not finite
            -- But we can use the bound: each RS ≥ -M'*(b-a)
            have h_lower_bound : ∀ r ∈ allRiemannSums P f', -M' * (I.b - I.a) ≤ r := by
              intro r hr
              rcases hr with ⟨t', ht', rfl⟩
              have : ∀ i : Fin n, -M' * P.delta i ≤ f' (t' i) * P.delta i := by
                intro i
                have h_bound : |f' (t' i)| ≤ M' := hM' (t' i) (hx_in_I (t' i) ?_)
                have h_nonneg_d : 0 ≤ P.delta i := h_delta_nonneg P i
                have h_f_ge : -M' ≤ f' (t' i) := by linarith [abs_le.mp h_bound]
                nlinarith
              calc
                -M' * (I.b - I.a) = ∑ i : Fin n, (-M' * P.delta i) := by
                  -- need sum of delta = b - a
                  sorry
                _ ≤ ∑ i : Fin n, (f' (t' i) * P.delta i) := Finset.sum_le_sum (fun i hi => this i)
                _ = (P.withTags t' ht').RiemannSum f' := rfl
            refine ⟨-M' * (I.b - I.a), ?_⟩
            intro r hr
            exact h_lower_bound r hr
          ) h_mem
        nlinarith
      · -- sum of infs ≤ sInf S: each Riemann sum ≥ sum of infs
        apply le_csInf ?_
        · have h_nonempty : (allRiemannSums P f').Nonempty := by
            refine ⟨P.RiemannSum f', ?_⟩
            refine ⟨P.x_tag, P.x_tag_between, rfl⟩
          exact h_nonempty
        · intro r hr
          rcases hr with ⟨t, ht, rfl⟩
          refine Finset.sum_le_sum (fun i hi => ?_)
          have h_tag_in_J : t i ∈ Set.Ico (P.x i.castSucc) (P.x i.succ) :=
            Set.mem_Ico.mpr ⟨(ht i).1, (ht i).2⟩
          have h_inf_le : sInf (f' '' Set.Ico (P.x i.castSucc) (P.x i.succ)) ≤ f' (t i) :=
            csInf_le (Set.bddBelow_image (hM' (t i) ?_)) (Set.mem_image.mpr ⟨t i, h_tag_in_J, rfl⟩)
          exact mul_le_mul_of_nonneg_right h_inf_le (h_delta_nonneg P i)
    sorry
