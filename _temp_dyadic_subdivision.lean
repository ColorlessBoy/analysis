...
lemma DyadicCube.subdivide_next {d : ℕ} (n : ℤ) (a : Fin d → ℤ) :
  ...
  · intro hx
    ...
    · simp [DyadicCube, Box.mem_toSet, BoundedInterval.set_Icc]
      ...
      have h_bound : (a i : ℝ) / ((2 : ℝ)^n) = ((2 : ℝ)*(a i : ℝ)) / ((2 : ℝ)^(n+1 : ℤ)) := by
        field_simp [show (2 : ℝ)^(n : ℤ) ≠ 0 from by positivity, show (2 : ℝ)^(n+1 : ℤ) ≠ 0 from by positivity]
        ring
      dsimp [k]
      split_ifs with h
      · -- k i = 0
        have h_low : ((2 : ℤ) * a i + 0 : ℝ) / ((2 : ℝ)^(n+1 : ℤ)) ≤ x i := by
          calc
